...
class Platform(Sprite):
    ...
    def update(self, player, moeda_group, contador, puloduplo_group ):   # remove ou acrescenta plataformas 
        ...
        if rd.randint(0,10000) < 1:
            tempo_extra = Relogio('images/relogio.png', contador)
            ...
            Relogio.clock_group.add(tempo_extra)

        # aparecimento de pulo duplo em plataformas
        if rd.randint(0,5000) < 1: 
            for _ in range(rd.randint(1,3)):
                pulo = PuloDuplo('images/bota.png')
